...
open class SNFilter: UIImageView {
    ...
    open static func generateFilters(_ originalImage: SNFilter, filters:[String]) -> [SNFilter] {
        ...
        let qos_attr = dispatch_queue_attr_make_with_qos_class(DispatchQueue.Attributes(), DispatchQoS.QoSClass.utility, 0)
        let syncQueue = DispatchQueue(label: "com.snapsliderfilters.app", attributes: qos_attr)
